...
public class MyLeaderboard : MonoBehaviour
{
    ...
    {
        LoadSpecificLeadboard(MySaveManager.Instance.Mode);
    }
    ...
    public void SetLeaderboardEntry()
    {
        ...
        if (autoLoadBoard)
        {
            gameMode = MySaveManager.Instance.Mode;
        }
        else
        {
            gameMode = mode;
        ...
        LeaderboardCreator.UploadNewEntry(key, inputField.text, MySaveManager.Instance.GetHighScore(gameMode), ((msg) =>
        {
            ClearLeaderboard();
